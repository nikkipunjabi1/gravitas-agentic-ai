...
| **P1.18** | feature flags + custom-component scaffolding | Three boolean toggles in `/admin/settings → Features`: master `feature_audit_enabled` (off = "chatbot only" — Discovery → END, no audit/strategy/mapping/output canvas), `feature_audit_use_psi`, `feature_audit_use_playwright` (sub-switches to pick which data engine fires when audit is on). Graph route gate flips when master is off; worker skips engines per sub-flags. New `ContactCard` canvas component as both a real surface (emitted by Output node alongside the close) AND the canonical "how to plug in a custom component" example — schema branch + component file + registry entry + agent emit-site, with the four-step recipe inline in `src/canvas/schema.ts`. |
| **P1.19** | AI-content disclaimer + doc sync | Admin-tunable disclaimer text ("AI-generated responses may contain mistakes. Verify key details before acting.") shown below the chat composer in embed mode + in the footer of the standalone `/copilot` route. Default in code via `DEFAULT_UI_DISCLAIMER` in `runtime-config.ts`; override from `/admin/settings → Branding` field; empty saved value falls back to default. Docs sweep: ROADMAP.md / CLAUDE.md / ADMIN_PANEL.md / UI_CONTRACT.md / AGENTS.md updated to reflect P1.18 + P1.19. |
