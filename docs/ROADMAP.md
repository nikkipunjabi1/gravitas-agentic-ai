...
- **Gravitas KB ingest pipeline (Phase 1):**
  ...
  - **Scheduled daily incremental refresh** — Vercel Cron (or equivalent) hits `worker/kb/refresh` at 04:00 UTC. Job fetches `sitemap.xml`, diffs `lastmod` + content hash against the `kb_documents` table in Supabase, re-embeds **only changed pages**.
